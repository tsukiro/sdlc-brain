...
## [2026-05-14] ingest | Prompt Engineering Guide — repositorio completo DAIR-AI (4 source-summaries)
- Secciones procesadas del repositorio https://github.com/dair-ai/Prompt-Engineering-Guide: Introduction, Risks, Applications (Data/Code/CaseStudy), Models
- **Introduction:** LLM Settings (Temperature/Top P/Max Length/Stop Sequences/Frequency vs Presence Penalty — distinción proporcional vs uniforme), Prompt Elements (4 componentes: Instruction/Context/Input Data/Output Indicator), Tips (Do vs. Don't efecto perverso de prohibiciones; especificidad; verbos de comando)
- **Risks:** Factualidad/alucinaciones (grounding + temperatura baja + few-shot con preguntas irrespondibles); Distribution Bias (experimento 8:2 → clasificación opuesta); Order Bias; Adversarial Prompting (Prompt Injection, Prompt Leaking, DAN, **Waluigi Effect**: RLHF codifica implícitamente comportamientos opuestos — consecuencia estructural no parcheable); defenses table (5 categorías, ninguna definitiva)
- **Applications-Data:** Generación de datos sintéticos; Synthetic RAG Dataset (Dai et al. 2022: 8 ejemplos manuales → near-SOTA; ~$55 para 50,000 docs); Code Gen (failure mode: imports omitidos; workflow SQL); Case Study (Clavié et al. 2023 arXiv:2303.07142: F1 65.6 → 91.7; hallazgo contraintuitivo: CoT few-shot PEOR que zero-shot en tareas de clasificación no-experta; 13 modificaciones acumulativas)
- **Models:** ChatGPT (RLHF; Role Prompting 2 componentes: Intent+Identity; gpt-3.5-turbo 90% más barato); GPT-4 (128K tokens; JSON Mode; `seed` para reproducibilidad; `system_fingerprint`; system messages resisten user overrides); LLaMA (desafía Chinchilla: optimiza inference budget no training budget; LLaMA-13B supera GPT-3-175B en 1 GPU); Mistral 7B (GQA+SWA; supera todos los 13B models; 14-cat content moderation self-hosted; Apache 2.0)
- Tensión documentada (Clavié et al.): CoT few-shot PEOR que zero-shot → contradicción con "CoT mejora razonamiento" de [[concepts/planning]]. Resolución: CoT es técnica de razonamiento, no mejora universal — solo aporta cuando hay razonamiento complejo que descomponer.
- Páginas creadas: [[promptingguide-introduction]], [[promptingguide-risks]], [[promptingguide-applications-data]], [[promptingguide-models]], [[concepts/llm-settings]]
- Páginas actualizadas: [[concepts/prompt-engineering]] (Elementos del prompt + 4 Tips incluyendo Do vs. Don't), [[concepts/rag]] (sección Synthetic RAG Dataset Dai et al. 2022), [[overview]] (stats 64 fuentes / 92 páginas; tema 3b expandido; tensiones 19-20), [[index]]
